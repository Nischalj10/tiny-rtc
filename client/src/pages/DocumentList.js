...
const Collaborator = ({ username, access, onEdit }) => {
  const editProps =
    typeof onEdit === "function" ? { action: true, onClick: onEdit } : {};
  return (
    <ListGroup.Item variant={ACCESS_VARIANT[access]} {...editProps}>
      {username}
      <Badge variant={ACCESS_VARIANT[access]} className="ml-1">
        {ACCESS_TEXT[access]}
      </Badge>
    </ListGroup.Item>
  );
...
const Doc = ({ documentId, username }) => {
  ...
  const { collaborators, access, requestCollaborators } = useCollaborators({
    documentId,
    username,
  });
  ...
  // modal data
  const users = usePossibleCollaborators({
    collaborators,
    needed: addCollaborator,
  });
      ...
      <td>
        <ListGroup>
          {others.map((c) => (
            <Collaborator
              key={c.username}
              onEdit={isManager ? () => setEditCollaborator(c) : null}
              {...c}
            />
          ))}
          {isManager && (
        ...
        <NewCollaboratorModal
          ...
          users={users}
          title={title}
          onUpdate={updateCollaborators}
          onHide={() => setAddCollaborator(false)}
        />
        <EditCollaboratorModal
          collaborator={editCollaborator}
          title={title}
          onUpdate={updateCollaborators}
          onHide={() => setEditCollaborator(undefined)}
        />
      </td>
    </tr>
  );
  ...
  return (
    <>
      ...
      <Table striped bordered hover>
        ...
        <tbody>
          {documents.map((docUuid) => (
            <Doc key={docUuid} documentId={docUuid} username={username} />
          ))}
        </tbody>
      </Table>
      ...
      <NewDocModal
        ...
        cancelNewDocument={cancelNewDocument}
      />
    </>
  );
}
