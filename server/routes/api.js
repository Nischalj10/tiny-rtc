 ...
 */
const ACCESS_LOOKUP = Object.fromEntries(
  Object.entries(ACCESS).map((entry) => entry.reverse())
);
 ...
 */
const permissionsToAccess = (permissions) =>
  ACCESS_LOOKUP[permissions] ??
  ACCESS_ORD.find((r) => (permissions & ACCESS[r]) === ACCESS[r]) ??
  "none";
...
const getPermissions = async (req) => {
  ...
  if (document === undefined)
    throw new Error("Path must contain parameter documentUuid");
  ...
  if (permissions === 0 || (permissions & permission) !== permission) {
    return res
      ...
      .json({
        success: false,
        message:
          "Document either does not exist or the user does not have the permission required.",
      });
  }
  next();
};
...
const hasLock = async (req, res, next) => {
  ...
  if (rows.length !== 1 || rows[0].lockUser !== user) {
    return res
      ...
      .json({
        success: false,
        message: "User must acquire a lock on the document.",
      });
  }
  if (DateTime.fromISO(rows[0].lockTime) < oldest_valid_lock) {
    return res
      ...
      .json({
        success: false,
        message:
          "User must re-acquire the lock on the document as it has expired.",
      });
  }
  next();
};
...
router.post("/jwt", async function (req, res, next) {
  ...
  if (typeof username !== "string" && /^[a-zA-Z0-9~_.-]+$/.test(username)) {
    return res
      ...
      .json({
        success: false,
        message:
          "The username must be a string matching the pattern /^[a-zA-Z0-9~_.-]+$/ .",
      });
  }
  if (typeof password !== "string" || passport.length === 0) {
    return res
      ...
      .json({
        success: false,
        message: "The password property must be a non-empty string.",
      });
  }
...
router.post("/users", async function (req, res, next) {
  ...
  if (typeof fullName !== "string" || fullName.length === 0) {
    return res
      ...
      .json({
        success: false,
        message: "The fullName property must be a non-empty string.",
      });
  }
...
router.post("/documents", isAuthenticated, async function (req, res, next) {
  ...
  await req.db
    .insert({
      document: uuid,
      user: username,
      permissions: PERMISSION_MANAGE | PERMISSION_WRITE | PERMISSION_READ,
    })
    .into("collaborators");
  ...
  async function (req, res, next) {
    ...
    if (typeof release !== "boolean") {
      res
        ...
        .json({
          success: false,
          message: "The release option must be true or false if provided.",
        });
      return;
    }
    ...
    if (release) {
      ...
      await req
        ...
        .andWhere(function () {
          ...
          this.whereNull("lockUser")
            .orWhere("lockUser", user)
            .orWhere("lockTime", "<", oldestValidLock);
        })
...
router.put(
  ...
  async function (req, res, next) {
    ...
    if (typeof content !== "string") {
      res
        .status(400)
        .json({ success: false, message: "The content is required." });
      return;
    }
  ...
  async function (req, res, next) {
    ...
    if (!Object.keys(ACCESS).includes(access)) {
      return res
        ...
        .json({
          success: false,
          message:
            'The access must be one of: "' + ACCESS_ORD.join('", "') + '".',
        });
    }
    ...
    if (userExists.length !== 1) {
      return res
        ...
        .json({
          success: false,
          message: "The username does not refer to a user that exists.",
        });
    }
    ...
    const existing = await req.db
      .select("*")
      .from("collaborators")
      .where({ document: req.params.documentUuid, user: user });
    ...
    if (existing.length > 0) {
      ...
      await req.db
        .insert({ document, user, permissions })
        .into("collaborators");
    }
    // return success
